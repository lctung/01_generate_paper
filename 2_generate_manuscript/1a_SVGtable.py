...
font_setting = [
    ...
    {"name": "BabelStoneHan", "path": f"{config.DIR_FONTS}/BabelStoneHan.ttf"},
    {"name": "GenSekiGothic-M", "path": f"{config.DIR_FONTS}/GenSekiGothic-M.ttc"},
    {"name": "SourceHanSansTC-VF", "path": f"{config.DIR_FONTS}/SourceHanSansTC-VF.ttf"},
    {"name": "NotoSansSymbols", "path": f"{config.DIR_FONTS}/NotoSansSymbols-VariableFont_wght.ttf"},
    {"name": "NotoSansThai", "path": f"{config.DIR_FONTS}/NotoSansThai-VariableFont_wdth,wght.ttf"},
    {"name": "NotoSansSymbols2", "path": f"{config.DIR_FONTS}/NotoSansSymbols2-Regular.ttf"},
    {"name": "NotoSansArabic", "path": f"{config.DIR_FONTS}/NotoSansArabic-VariableFont_wdth,wght.ttf"},
    {"name": "NotoSansMongolian", "path": f"{config.DIR_FONTS}/NotoSansMongolian-Regular.ttf"},
    {"name": "STIX", "path": f"{config.DIR_FONTS}/STIXGeneral.otf"},
    {"name": "DejaVuSans", "path": f"{config.DIR_FONTS}/DejaVuSans.ttf"},
    {"name": "Quivira", "path": f"{config.DIR_FONTS}/Quivira.otf"},
    {"name": "Symbola_hint", "path": f"{config.DIR_FONTS}/Symbola_hint.ttf"},
    {"name": "Symbola", "path": f"{config.DIR_FONTS}/Symbola.ttf"},
    {"name": "Symbola-AjYx", "path": f"{config.DIR_FONTS}/Symbola-AjYx.ttf"},
    {"name": "FreeSerif.ttf", "path": f"{config.DIR_FONTS}/FreeSerif.ttf"},
    {"name": "Symbola-12", "path": f"{config.DIR_FONTS}/Symbola-12.ttf"},
    {"name": "NotoSans-Italic-VariableFont_wdth,wght", "path": f"{config.DIR_FONTS}/NotoSans-Italic-VariableFont_wdth,wght.ttf"},
    {"name": "NotoSans-VariableFont_wdth,wght", "path": f"{config.DIR_FONTS}/NotoSans-VariableFont_wdth,wght.ttf"},
    {"name": "NotoSans-Regular", "path": f"{config.DIR_FONTS}/NotoSans-Regular.ttf"},
...
for setting in font_setting:
    ...
    if os.path.exists(path):
        try:
            ...
            # 判斷是否為 TTC (字體集)，若是則根據需要選 index，否則選 0
            is_ttc = path.lower().endswith(".ttc")
            target_idx = 1 if (is_ttc and "GenSekiGothic" in setting["name"]) else 0
            
            ttfont = TTFont(path, fontNumber=target_idx)
            ...
            print(f"載入字體 {setting['name']} 失敗: {e}")

noto_prop = next((obj["prop"] for obj in font_objects if obj["name"] == "NotoSansTC"), None)
#noto_prop = next((obj["prop"] for obj in font_objects if obj["name"] == "GenSekiGothic-M"), None)#

def get_best_font_prop(char):
    if not char: return noto_prop
    ...
    # 如果都找不到，印出警示並回傳預設
    print(f"警告: 字元 {char} (U+{target_unicode:04X}) 在所有設定字體中都找不到")
    return font_objects[0]["prop"] if font_objects else noto_prop
    ...
    with open(file, 'r', encoding='utf-8') as f:
        try:
            ...
            for i in range(total_characters):
                try:
                    # 提取原始 Hex 字串 (例如從 "U+28000" 提取 "28000")
                    raw_hex = p["CP950"][i]["UNICODE"].replace("U+", "").replace("0x", "")
                    
                    # 根據長度決定使用 \u (4位) 或 \U (8位)
                    if len(raw_hex) > 4:
                        v[i] = "\\U{:08x}".format(int(raw_hex, 16))
                    else:
                        v[i] = "\\u{:04x}".format(int(raw_hex, 16))
                except:
                    v[i] = ""
            return v
        except Exception as e:
    ...
    for j in range(10):
        for i in range(10):
            ...
            if char_code == "123" or not char_code:
                ...
                # 畫主字
                plt.text(X[i] + 1, Y[j] - 3, actual_char,
                        fontsize=14, color="black", fontproperties=best_prop)
                
                # --- 修改後的編碼標籤邏輯 ---
                code_point = ord(actual_char)
                if code_point <= 0xFFFF:
                    # 4位數以內（含4位），補滿 4 位大寫 Hex
                    raw_hex = "{:04X}".format(code_point)
                else:
                    # 超過 4 位（如 U+20000 以上），顯示實際位數（通常為 5 位）
                    raw_hex = "{:X}".format(code_point)
                # --------------------------

                plt.text(X[i] + 8.5, Y[j] - 3, raw_hex,
                        fontsize=8, color="black", fontproperties=noto_prop)
                
                fnip[page][index] = raw_hex
